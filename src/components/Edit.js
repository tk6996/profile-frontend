...
export default function Edit() {
  ...
  const submitForm = async (data) => {
    try {
      return await axios.put(
        "http://localhost:7000/api/edit" + location.search,
        data,
        {
  ...
  useEffect(() => {
    ...
    if (!found) {
      setLoading(true);
      axios
        .get("http://localhost:7000/api/edit" + location.search, {
          withCredentials: true,
        })
        .then((result) => {
          if (result.data.find) {
    ...
    return () => {
      ...
      setLoading(false);
      if (finish)
        if (found) activeFlash(dispatch, SUCCESS, "Profile editor");
  ...
  return (
    <div className="container">
      <h1>Editing Profile for Watcharin</h1>
      <Flash />
      <form onSubmit={(event) => event.preventDefault()}>
        ...
        <div className="form-button">
          <input type="submit" value="Edit" onClick={sendProfile} />
          <input
            type="button"
            value="Cancel"
